"""
PPG ADC Data Logger — CSV Export
==================================
Reads filtered ADC values streamed by the Arduino sketch (ppg_reader.ino)
and logs them to a timestamped CSV file for a user-defined duration.

CSV columns:
    sample_index, elapsed_time_s, adc_value
...
Usage:
    ...
    python ppg_logger.py --duration 10 --out my_data.csv
"""
...
# ── Argument parsing ──────────────────────────────────────────────────────────
def parse_args():
    p = argparse.ArgumentParser(description="PPG ADC CSV Logger")
    p.add_argument("--duration", type=float, required=True,
                   help="Recording duration in seconds (set by you)")
    p.add_argument("--port", default=None,
                   help="Serial port (auto-detect if omitted)")
    p.add_argument("--out", default=None,
                   help="Output CSV filename (default: ppg_data_<timestamp>.csv)")
    return p.parse_args()
...
def main():
    ...
    out_file = args.out or f"ppg_data_{time.strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    try:
        with open(out_file, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["sample_index", "elapsed_time_s", "adc_value"])
            ...
            while True:
                ...
                writer.writerow([sample_index, f"{elapsed:.4f}", value])
                sample_index += 1

                # Lightweight progress indicator every ~1 second
                if sample_index % 100 == 0:
                    print(f"\r[INFO] {elapsed:5.1f}s elapsed | {sample_index} samples", end="")
    ...
    print(f"\n[INFO] Done. {sample_index} samples written to {out_file}")
